...
const Sidebar = ({ activeTab, setActiveTab }) => {
  const { logoutUser, user, getInitials, startupDetails, analysisScores, roadmapNodes } = useStartup();
  ...
  const navItems = [
    ...
    { id: 'history', label: 'Analysis History', icon: Clock, path: '/history' },
  ...
  const getIsSelected = (item) => {
    ...
    if (item.path === '/documents' && location.pathname === '/documents') return true;
    if (item.path === '/history' && location.pathname === '/history') return true;
  ...
  return (
    <aside className="h-full w-full lg:w-72 lg:border-r border-indigo-500/10 bg-[#0a0a0f]/95 flex flex-col justify-between p-5 shrink-0">
